...
<?php 
  ...
  /********************************************************************* */
   ...
   function get_breaking_news() {   
      ...
      $con=$db->connect_db();
      $stmt = $con->prepare(
         "SELECT `post_id`,`post_title`,`post_image` 
         FROM `tb_posts` WHERE `post_isbreaking`= ? ORDER BY `post_id` DESC LIMIT 4");
  ...
  /********************************************************************* */
   function visitorcount($page='Homepage') {
      ...
      $con = $db->connect_db();
      $stmt = $con->prepare(
         "INSERT INTO tb_visitorlog(access_page,access_counter,access_date) 
      values(?,?,?)");
 ...
 /********************************************************************* */
  ...
  /********************************************************************* */
   ...
   function save_comment($postid,$posttitle,$comment,$author,$email) {
      ...
      $stmt= $con->prepare(
         "INSERT INTO tb_comments(post_id,post_title,comment_body,comment_author,comment_date,comment_email) 
         values(?, ?, ?, ?, ?, ?)");
